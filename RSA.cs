...
namespace PR06
{
    public class RSA
    {
        ...
        // Простое число (часть закрытого ключа)
        ...
        public static ulong GetSimpleNumber(int min, int max)
        {
        ...
        public static void SetPartOfKeys(ulong p, ulong q)
        {
            if (p * q > 65 || p * q < 250 && p != q)
            {
                GetResultOfSimpleNumbers(p, q);
            }
            else
        ...
        public static void EncryptMessage(byte[] byteArray, string encryptedFileName)
        {
            ...
            File.WriteAllBytes(encryptedFileName, encryptCode.ToArray());
        }
        ...
        public static void DecryptMessage(byte[] byteArray, string decryptedFileName)
        {
            ...
            File.WriteAllBytes(decryptedFileName, buffer);
        }
    }
}
